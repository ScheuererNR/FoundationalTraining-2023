...
fcst_dir = '/home/michael/bigdisk2/pro/SFE/Systems_daily_nc/'   # directory where the netcdf forecast files are stored
...
nc = Dataset(output_dir+'percentiles_'+system+'_'+res_str+'_'+str(start_year)+'_'+str(end_year)+'_'+month_init_str+'.nc')
...
filename = output_dir+'percentiles_'+system+'_'+res_str+'_'+str(start_year)+'_'+str(end_year)+'_'+month_init_str+'.nc'
data_load = xr.open_dataset(filename, engine='netcdf4')
...
#dates_fcst = pd.DatetimeIndex(data_subset.forecast_time1.values/3600e9) - pd.Timedelta(days=1)
lead_time_fcst = data_subset.forecast_time1.values/3600e9
...
fcst_dir = '/home/ghacof/COF65/gcm/Aug2023/Rscripts/precip/c3S/'

data_load = xr.open_dataset(f'{fcst_dir}ecmwfForecastV51.grib', engine='cfgrib')

nmbs, nlts, nlatf, nlonf = prcp_fcst.shape

# Calculate forecasts of daily precipitation and convert to mm/m^2
prcp_fcst_daily = np.maximum(0., 1000.*np.diff(np.insert(prcp_fcst, 0, 0., axis=1), axis=1)).reshape((nmbs,nlts,nlatf,nlonf))
