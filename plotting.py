...
def plot_fields (fields_list, lon, lat, lon_bounds, lat_bounds, main_title, subtitle_list, unit, vmin=None, vmax=None, cmap='BuPu', water_bodies=False, ticks=None, tick_labels=None):
    ...
    if not ticks == None:
        if not isinstance(ticks[0], list):
            ticks = [ticks for i in range(n_img)]

    if not tick_labels == None:
        if not isinstance(tick_labels[0], list):
